...
import { setUiLang, t, UI_LANGS, uiLang } from "../i18n";
...
export function PrefsControls({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={`flex flex-col gap-2 ${compact ? "" : "min-w-[210px]"}`}>
      <div role="group" aria-label={t("Режим")} className="flex gap-1 rounded-[11px] bg-slate-100 p-1">
        {THEMES.map(({ id, icon: Icon, label }) => (
          <button key={id} type="button" aria-pressed={theme === id} title={t(label)} onClick={() => setThemePref(id)} className={seg(theme === id)}>
            <Icon size={14} />
            {!compact && <span>{t(label)}</span>}
          </button>
        ))}
      </div>
      <div role="group" aria-label={t("Интерфейс тілі")} className="flex gap-1 rounded-[11px] bg-slate-100 p-1">
        {UI_LANGS.map((l) => (
