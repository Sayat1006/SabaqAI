...
export function TelegramCard({ userId, isAdmin }: { userId: string; isAdmin: boolean }) {
  ...
  async function run(fn: () => Promise<void>) {
    ...
    try {
      ...
      setMsg({ ok: false, text: e instanceof Error ? e.message : tr("Әрекет орындалмады.") });
    } finally {
      setBusy(false);
    }
  }
  ...
  const check = () =>
    run(async () => {
      ...
      if (s?.linked) {
        setWaiting(false);
        setMsg({ ok: true, text: tr("Telegram қосылды!") });
      } else setMsg({ ok: false, text: tr("Әлі қосылмаған. Ботта «Start» (Бастау) батырмасын басыңыз.") });
    });
  ...
  const setup = () =>
    run(async () => {
      ...
      setBot(name);
      setMsg({ ok: true, text: tr("Бот іске қосылды: @{name}", { name }) });
    });

  return (
    <section className="rounded-[22px] border border-slate-200 bg-surface p-6 sm:p-7">
      <h2 className="flex items-center gap-2 text-lg font-bold">
        <Send size={18} className="text-violet-600" /> {tr("Telegram хабарламалары")}
      </h2>
      <p className="mt-1 text-sm text-slate-500">{tr("Оқушылар тест тапсырған сайын және тест жабылғанда Telegram-ға қысқа хабарлама келеді.")}</p>
      <p className="mt-1 text-sm text-slate-500">
        {tr("Ботта материал жасауға да болады:")} <code className="rounded bg-slate-100 px-1">/qmzh 5-сынып тақырып</code> — {tr("ҚМЖ")},{" "}
        <code className="rounded bg-slate-100 px-1">/test 7-сынып тақырып</code> — {tr("тест. Дайын Word файлы чатқа келеді және «Жобаларда» сақталады.")}
      </p>
      ...
      <div className="mt-4 flex flex-col gap-3">
        {status === undefined ? (
          ...
          <p className="rounded-xl bg-slate-50 px-3.5 py-2.5 text-sm text-slate-600">{tr("Telegram бот әлі бапталмаған. Әкімші ботты жасап, кілтін Supabase-ке қоюы керек.")}</p>
        ) : status.linked ? (
          <>
            <p className="inline-flex items-center gap-2 text-sm font-semibold text-fuchsia-800">
              <Check size={16} /> {tr("Қосылған:")} <a href={`https://t.me/${bot}`} target="_blank" rel="noopener noreferrer" className="text-violet-700 underline">@{bot}</a>
            </p>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={btn}
              >
                {status.notify ? <BellOff size={15} /> : <BellRing size={15} />} {status.notify ? tr("Хабарламаларды тоқтату") : tr("Хабарламаларды қосу")}
              </button>
              <button
                type="button"
                disabled={busy}
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={link} disabled={busy} className="inline-flex items-center gap-2 rounded-[12px] bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">
              <ExternalLink size={15} /> {tr("Telegram-ды қосу")}
            </button>
            {waiting && (
              <button type="button" onClick={check} disabled={busy} className={btn}>
                <RefreshCw size={15} /> {tr("Қосылғанын тексеру")}
              </button>
            )}
          </div>
        )}
        {waiting && !status?.linked && <p className="text-xs text-slate-500">{tr("Telegram ашылады — ботта «Start» (Бастау) басыңыз, сосын осында «Қосылғанын тексеру» батырмасын басыңыз.")}</p>}

        {isAdmin && (
          <div className="mt-2 rounded-xl border border-dashed border-slate-300 p-3.5">
            <div className="text-sm font-semibold">{tr("Әкімші: ботты іске қосу")}</div>
            <p className="mt-0.5 text-xs text-slate-500">{tr("Supabase-ке TELEGRAM_BOT_TOKEN құпиясын қойып, telegram-bot функциясын жариялаған (немесе жаңартқан) соң бір рет басыңыз.")}</p>
            <button type="button" onClick={setup} disabled={busy} className={`${btn} mt-2`}>
              <Settings size={15} /> {tr("Ботты іске қосу")}
            </button>
          </div>
        )}
        {msg && (
