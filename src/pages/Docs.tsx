...
export default function DocsPage() {
  ...
  useEffect(() => {
    ...
    getDocument(openedId)
      .then((d) => d && fill(d))
      .catch((e) => setError(e instanceof Error ? e.message : "Құжатты ашу мүмкін болмады."));
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    if (def.fields.includes("student") && student.trim().length < 2) return setError("Оқушының атын жазыңыз.");
    if (def.fields.includes("topic") && topic.trim().length < 3) return setError(`${def.topicLabel} жазыңыз.`);
    setError("");
    setGenerating(true);
    setEditing(false);
    scrollToResult();
    try {
      ...
      setError(err instanceof Error ? err.message : "Құжат жасау мүмкін болмады.");
    } finally {
      setGenerating(false);
    }
  }
  ...
  async function saveEdited(next: DocData) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Сақтау мүмкін болмады.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    if (!window.confirm("Құжатты жою керек пе?")) return;
    try {
  ...
  async function handleExport() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Word файлын жасау мүмкін болмады.");
    } finally {
      setExporting(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[1360px] px-4 py-9 sm:px-10 print:p-0">
      <PageHeader
        crumb="Құжаттар"
        title="Құжаттар"
        subtitle="Мінездеме, ата-ана жиналысының баяндамасы, тәрбие сағаты мен іс-шара сценарийі, тоқсандық есеп — деректерді жазыңыз, AI ресми құжатты дайындайды. Word-қа жүктеп, өңдей аласыз."
      />
        ...
        <div className="flex w-full flex-col gap-5 lg:sticky lg:top-24 lg:w-[400px] print:hidden">
          ...
          >
              ...
              <div className="grid grid-cols-2 gap-2">
                {DOC_TYPES.map((d) => (
                  ...
                  >
                    ...
                    <span className="text-[11.5px] leading-snug text-slate-500">{d.hint}</span>
                  </button>
                ))}
              </div>
            </fieldset>
            ...
            {def.fields.includes("student") && (
              <FormField label="Оқушының аты-жөні" hint="Құпиялылық үшін тек атын және тегінің бас әрпін жазсаңыз да болады — Word-та толықтырасыз.">
                <input value={student} onChange={(e) => setStudent(e.target.value)} maxLength={80} placeholder="Мыс.: Айгерім С." className={fieldClass} />
              </FormField>
            )}
            {def.fields.includes("topic") && (
              <FormField label={def.topicLabel}>
                <input value={topic} onChange={(e) => setTopic(e.target.value)} maxLength={200} placeholder={def.topicPh} className={fieldClass} />
              </FormField>
            )}
            ...
            <FormField label="Деректер мен тілектер" hint="Неғұрлым нақты жазсаңыз, құжат соғұрлым дәл шығады. AI тек осы деректерге сүйенеді.">
              <textarea rows={5} value={notes} onChange={(e) => setNotes(e.target.value)} maxLength={2000} placeholder={def.notesPh} className={`${fieldClass} resize-y`} />
            </FormField>
            <FormField label="Мұғалімнің аты-жөні (қолтаңбаға)">
            ...
            >
              ...
              {generating ? "Дайындалуда..." : "Құжат жасау"}
            </button>
          </form>
                  ...
                  <li key={d.id} className="flex items-center gap-2">
                    <button type="button" onClick={() => fill(d)} className="min-w-0 flex-1 rounded-lg px-2 py-1.5 text-left text-sm hover:bg-slate-50">
                      ...
                      <span className="block text-xs text-slate-500">
                        {docTypeOf(d.type).label} · {timeAgo(d.savedAt)}
                      </span>
                    </button>
                    <button type="button" onClick={() => handleDelete(d.id)} aria-label={`Жою: ${d.title}`} className="rounded-lg p-1.5 text-slate-500 hover:bg-rose-50 hover:text-rose-700">
                      <X size={15} />
                    </button>
                  </li>
                ))}
        ...
        <section id="result" className="min-w-0 flex-[1_1_560px] scroll-mt-28" aria-live="polite">
          {generating ? (
            ...
            <div className="flex min-h-[420px] flex-col items-center justify-center gap-3.5 rounded-3xl border-2 border-dashed border-slate-200 p-10 text-center text-slate-500">
              ...
              <div className="text-base text-slate-900">Құжат түрін таңдап, деректерді жазыңыз</div>
              <div className="text-[13.5px]">Дайын құжат осы жерде пайда болады және «Жобалар» бөлімінде сақталады.</div>
            </div>
          ) : editing ? (
            ...
            <>
              <div className="mb-4 flex flex-wrap gap-2 print:hidden">
                <button type="button" onClick={handleExport} disabled={exporting} className={ghost}>
                  <Download size={16} /> {exporting ? "Дайындалуда..." : "Word (.docx)"}
                </button>
                <button type="button" onClick={() => window.print()} className={ghost}>
                  <Printer size={16} /> Басып шығару / PDF
                </button>
...
function DocEditor({ doc, saving, onSave, onCancel }: { doc: DocData; saving: boolean; onSave: (d: DocData) => void; onCancel: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-[22px] border border-slate-200 bg-white p-5 sm:p-7">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={save} disabled={saving} className="inline-flex items-center gap-2 rounded-[11px] bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">
            <Check size={16} /> {saving ? "Сақталуда..." : "Сақтау"}
          </button>
        </div>
      </div>
      ...
      <FormField label="Атауының астындағы жол">
      ...
      {sections.map((s, i) => (
        <fieldset key={i} className="flex flex-col gap-3 rounded-2xl border border-slate-200 p-4">
          <div className="flex items-center gap-2">
            <input value={s.heading} onChange={(e) => patch(i, { heading: e.target.value })} aria-label={`${i + 1}-бөлім атауы`} placeholder="Бөлім атауы" className={`${fieldClass} font-semibold`} />
            <button
              type="button"
              onClick={() => setSections((all) => all.filter((_, j) => j !== i))}
              aria-label={`${i + 1}-бөлімді өшіру`}
              className="rounded-lg p-2 text-slate-500 hover:bg-rose-50 hover:text-rose-700"
            >
              <X size={16} />
            </button>
          </div>
          <FormField label="Мәтін" hint="Азат жолдарды бос жолмен бөліңіз.">
          ...
          <FormField label="Тізім" hint="Әр тармақ — жаңа жолдан.">
                    ...
                    <tr key={rr}>
                      {r.map((c, cc) => (
                        <td key={cc} className="border border-slate-200 p-0">
                          <input value={c} onChange={(e) => setCell(i, rr, cc, e.target.value)} aria-label={`Кесте ${rr + 1}:${cc + 1}`} className="w-full min-w-[80px] bg-transparent px-2 py-1 outline-none focus:bg-violet-50" />
                        </td>
                      ))}
                    </tr>
                  ))}
