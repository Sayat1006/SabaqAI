...
export default function ProfilePage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (form!.name.trim().length < 2) {
      setStatus({ ok: false, text: "Аты-жөніңізді жазыңыз." });
      return;
    }
    setSaving(true);
    setStatus(null);
    try {
      await save({ ...form!, name: form!.name.trim() }, "Мәліметтер сақталды.");
    } catch (err) {
      setStatus({ ok: false, text: err instanceof Error ? err.message : "Сақтау мүмкін болмады." });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleAvatar(file: File | null) {
    ...
    if (!file.type.startsWith("image/")) {
      setStatus({ ok: false, text: "Сурет файлын таңдаңыз (JPG, PNG)." });
      return;
    }
    if (file.size > 15 * 1024 * 1024) {
      setStatus({ ok: false, text: "Сурет тым үлкен (15 МБ-тан аспауы керек)." });
      return;
    }
    setAvatarBusy(true);
    setStatus(null);
    try {
      ...
      await save({ ...toInput(user!), avatarUrl }, "Аватар жаңартылды.");
      setForm({ ...form!, avatarUrl });
    } catch (err) {
      setStatus({ ok: false, text: err instanceof Error ? err.message : "Суретті жүктеу мүмкін болмады." });
    } finally {
      setAvatarBusy(false);
  ...
  async function removeAvatar() {
    setAvatarBusy(true);
    try {
      await save({ ...toInput(user!), avatarUrl: "" }, "Аватар өшірілді.");
      setForm({ ...form!, avatarUrl: "" });
    } catch (err) {
      setStatus({ ok: false, text: err instanceof Error ? err.message : "Өшіру мүмкін болмады." });
    } finally {
      setAvatarBusy(false);
    }
  }
  ...
  async function handlePassword(e: React.FormEvent) {
    ...
    setPwStatus(null);
    if (pw.next.length < 8) {
      setPwStatus({ ok: false, text: "Жаңа құпия сөз кемінде 8 таңбадан тұруы керек." });
      return;
    }
    if (pw.next !== pw.repeat) {
      setPwStatus({ ok: false, text: "Жаңа құпия сөздер сәйкес емес." });
      return;
    }
    setPwBusy(true);
    try {
      ...
      setPwStatus({ ok: true, text: "Құпия сөз өзгертілді." });
    } catch (err) {
      setPwStatus({ ok: false, text: err instanceof Error ? err.message : "Құпия сөзді өзгерту мүмкін болмады." });
    } finally {
      setPwBusy(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-4 py-9 sm:px-10">
      <PageHeader crumb="Жеке бет" title="Жеке бет" subtitle="Өзіңіз туралы мәліметтерді толтырыңыз: олар ҚМЖ мен тесттерге автоматты түрде қойылады." />
      ...
      <div className="mt-8 flex flex-col gap-6">
        ...
        <section className={`${cardClass} flex flex-wrap items-center gap-6`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-semibold text-violet-700">{user.role === "admin" ? "Әкімші" : "Мұғалім"}</span>
              {user.subject && <span className="rounded-full bg-fuchsia-100 px-3 py-1 text-xs font-semibold text-fuchsia-700">{user.subject}</span>}
        ...
        <form onSubmit={handleSave} noValidate className={`${cardClass} flex flex-col gap-5`}>
          <h2 className="text-lg font-bold">Жеке мәліметтер</h2>
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <Field label="Мектеп">
              <input value={form.school} onChange={(e) => set("school", e.target.value)} maxLength={200} placeholder="мыс.: №25 мектеп-гимназия" className={fieldClass} />
            </Field>
            <Field label="Біліктілік санаты">
            ...
            <Field label="Педагогикалық өтілі (жыл)">
              <input
                type="number"
                min={0}
                max={70}
          ...
          <div>
            <span className="mb-2 block text-[13px] font-semibold text-slate-500">Сабақ беретін сыныптарым</span>
            <div role="group" aria-label="Сабақ беретін сыныптар" className="grid grid-cols-6 gap-2 sm:grid-cols-11">
              {GRADES.map((g) => {
            ...
            <span className="mt-1.5 block text-xs text-slate-500">Бірінші таңдалған сынып ҚМЖ мен тест беттерінде әдепкі болып тұрады.</span>
          </div>

          <Field label="Өзім туралы">
            <textarea value={form.bio} onChange={(e) => set("bio", e.target.value)} maxLength={1000} rows={3} placeholder="Қысқаша: тәжірибеңіз, қызығушылықтарыңыз, жетістіктеріңіз" className={`${fieldClass} resize-y`} />
          </Field>
          ...
          <div>
            <button type="submit" disabled={saving} className="inline-flex items-center gap-2.5 rounded-[14px] bg-violet-600 px-6 py-3.5 text-[15px] font-semibold text-white transition hover:-translate-y-0.5 disabled:translate-y-0 disabled:opacity-70">
              <Save size={16} /> {saving ? "Сақталуда..." : "Сақтау"}
            </button>
          </div>
        </form>
        ...
        <form onSubmit={handlePassword} noValidate className={`${cardClass} flex flex-col gap-5`}>
          <h2 className="text-lg font-bold">Құпия сөзді өзгерту</h2>
          <input type="text" autoComplete="username" value={user.email} readOnly hidden />
          <div className="grid gap-5 sm:grid-cols-3">
            <Field label="Қазіргі құпия сөз">
          ...
          <div>
            <button type="submit" disabled={pwBusy || !pw.current || !pw.next} className="inline-flex items-center gap-2.5 rounded-[14px] bg-navy-900 px-6 py-3.5 text-[15px] font-semibold text-white disabled:opacity-60">
              <KeyRound size={16} /> {pwBusy ? "Өзгертілуде..." : "Құпия сөзді өзгерту"}
            </button>
          </div>
        </form>
      </div>
